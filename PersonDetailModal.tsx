...
// FIX: Added currentUser prop to handle role-based logic for interaction types.
        ...
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex items-center justify-between mt-2">
                    {/* FIX: Conditionally render interaction type selector based on user role */}
...
export const PersonDetailModal: React.FC<PersonDetailModalProps> = ({ person, onClose, currentUser, touchpoints, notes, activityLogs, onAddInteraction, onUpdatePersonDetails, onStartArchive }) => {
  ...
  // FIX: Updated leaders filter to include Usuario role
  const leaders = mockUsers.filter(u => u.role === UserRole.Lider || u.role === UserRole.Usuario);
  
  // FIX: Added canManagePerson for permission checks
  ...
  const handleEditChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'age') finalValue = parseInt(value, 10) || 0;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      ...
      >
        <header className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center flex-shrink-0">
            ...
            <div className="flex items-center space-x-2">
                {isEditing ? (
                    ...
                    <>
                      {/* FIX: Conditionally render archive button */}
                      ...
                      {/* FIX: Conditionally render edit button */}
                      {canManagePerson && (
        ...
        <main className="p-6 flex-1 overflow-y-auto">
            {/* FIX: Conditionally render NextBestAction */}
                        ...
                        <>
                            ...
                            <div>
                             ...
                             <div>
                                ...
                                <input type="number" name="age" value={editablePerson.age} onChange={handleEditChange} className="mt-1 block w-full input-style"/>
                            </div>
                            <div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="mt-1 space-x-4">
                                    <label><input type="radio" name="accepts_visit" checked={editablePerson.accepts_visit === true} onChange={() => setEditablePerson(p => ({...p, accepts_visit: true}))} className="mr-1"/> Sim</label>
                                    <label><input type="radio" name="accepts_visit" checked={editablePerson.accepts_visit === false} onChange={() => setEditablePerson(p => ({...p, accepts_visit: false}))} className="mr-1"/> Não</label>
                                </div>
                            </div>
            ...
            <div role="tabpanel" tabIndex={0} hidden={activeTab !== 'interacoes'}>
              {activeTab === 'interacoes' && (
                  <div>
                    {/* FIX: Pass currentUser to AddInteraction */}
